...
with the suboptimal bandwidth rule \(h_m=m^{-0.025}\), where \(m\asymp n\) is
the training-stratum sample size, rather than bandwidth selection by
cross-validation. Since \(W_2\) is binary and the kernel estimator is stratified
by \(W_2\) (and by treatment level for the outcome regression), the
nonparametric smoothing problem is one-dimensional in \(W_1\); for
twice-smooth nuisance functions, the consistently estimated nuisance has the
usual kernel RMSE rate \(m^{-2/5}\), whereas the slow bandwidth rule gives RMSE
rate \(m^{-0.05}\). The resulting product rate is \(m^{-9/20}\), slower than
\(m^{-1/2}\). We report the complete cells with
