...
- `inference = "wald"`; for binary treatment contrasts this uses the corrected level-set Wald standard error by default.
- `inference = "bootstrap"`

Practical guidance:

- Use the default jackknife intervals for standard calibrated DML.
- Use Wald when both nuisance estimators are consistent, even if one converges arbitrarily slowly. The default `wald_correction = "auto"` uses an unconstrained empirical level-set correction for the binary contrast. Set `wald_correction = "none"` to recover the standard Wald standard error.
- Set `wald_conservative = TRUE` in R or `wald_conservative=True` in Python to use the maximum of the standard Wald and corrected Wald standard errors.
